...
export const useEstiplanStore = create<EstiplanStore>()((set, get, api) => ({
  ...
  autoLayout: () => {
    const state = get();
    const rfNodes = state.getRfNodes().filter((n) => n.type === 'variable');
    const rfEdges = state.getRfEdges();
    const layouted = applyDagreLayout(rfNodes, rfEdges, state.flowDirection);
